...
describe("<Events />", () => {
  ...
  describe("The log type selector", () => {
    test("should have Events as default log type upon render", () => {
      expect(screen.getByRole('combobox', {name:'log-type'})).toHaveDisplayValue('Events');
      expect(screen.getByRole('combobox', {name:'log-type'})).not.toHaveDisplayValue('Alerts');
      expect(screen.getByRole('combobox', {name:'log-type'})).not.toHaveDisplayValue('OOMKills');
    });

    test("should change displayed value based on user input", () => {

      // change the selection to 'alerts'
      fireEvent.change(screen.getByRole('combobox', {name:'log-type'}), {target: {value: 'alerts'}});
      
      // the screen should correctly display the newly chosen option
      expect(screen.getByRole('combobox', {name:'log-type'})).toHaveDisplayValue('Alerts');
      expect(screen.getByRole('combobox', {name:'log-type'})).not.toHaveDisplayValue('Events');
      expect(screen.getByRole('combobox', {name:'log-type'})).not.toHaveDisplayValue('OOMKills');

      // change the selection to 'OOMKills'
      fireEvent.change(screen.getByRole('combobox', {name:'log-type'}), {target: {value: 'oomkills'}});
      
      // the screen should correctly display the newly chosen option
      expect(screen.getByRole('combobox', {name:'log-type'})).toHaveDisplayValue('OOMKills');
      expect(screen.getByRole('combobox', {name:'log-type'})).not.toHaveDisplayValue('Events');
      expect(screen.getByRole('combobox', {name:'log-type'})).not.toHaveDisplayValue('Alerts');
    });
